package com.example.QuanLyGiaoDich.TableSpace;
...
public class TablespaceService {
    ...
    public List<TablespaceInfo> getTablespaceInfo() {
        return jdbcTemplate.execute(
            conn -> {
                CallableStatement stmt = conn.prepareCall("{ call SYS.GET_TABLESPACE_INFO(?) }");
    	...
    	try {
            jdbcTemplate.execute(
                (Connection connection) -> {
                    CallableStatement cs = connection.prepareCall("{ call SYS.CREATE_TABLESPACE_USER_CHOICE(?, ?, ?) }");
    ...
    public void addDatafileToTablespace(String tablespaceName, String datafilePath, int size) {
        try {
            jdbcTemplate.execute(
                (Connection connection) -> {
                    CallableStatement cs = connection.prepareCall("{ call SYS.ADD_DATAFILE_TO_TABLESPACE(?, ?, ?) }");
                    cs.setString(1, tablespaceName);
                    cs.setString(2, datafilePath);
                    cs.setInt(3, size);
                    return cs;
